...
def get_market_date(server_name):
    server_list = [
        "Deriv-Demo", "ForexTimeFXTM-Demo01", "ForexTimeFXTM-ECN-Demo", "ICMarketsSC-Demo02", "HugosWay-Demo03"
    ]
...
if __name__ == '__main__':
    ...
    market_watch_xpath = '//div[@class="page-window market-watch compact"]/div/div[@class="h"]'
        ...
        for login_details in account_details:
            ...
            try:
                ...
                WebDriverWait(browser, 20).until(
                    ec.presence_of_element_located((By.CSS_SELECTOR, login_css))).send_keys(
                    login_details[0])
                browser.implicitly_wait(10)
                WebDriverWait(browser, 30).until(ec.presence_of_element_located((By.CSS_SELECTOR, pass_css))).send_keys(
                    login_details[1])
                browser.implicitly_wait(10)
                ...
                if market_watch_time_ele is not None:
                    ...
                    if balance_equ_ele is not None:
                        ...
                        balance = balance_equity[1].replace('USD  Equity', '').replace(' ', '').strip()
                        equity = balance_equity[2].replace('Free margin', '').replace('Margin', '').replace(' ',
                                                                                                            '').strip()
                        result_dict['date'] = datetime.strptime(get_market_date(login_details[2]),
                                                                '%Y-%m-%d %H:%M:%S').astimezone(
                            pytz.timezone('Africa/Brazzaville')).strftime('%Y-%m-%d %H:%M:%S')
                    ...
                    print(f'Record inserter successfully. Inserted object id: {new_insert_id.inserted_id}')
                    con.close_con()
                else:
                ...
                print(f'Authorization Failed for {login_details[0]} account number. Trying again..')
                browser.quit()
